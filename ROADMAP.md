...
Current runtime support can verify carved tensors reconstruct source dense FFN weights and can execute all carved experts as a gated MLP parity layer. It can also execute selected expert subsets from document-pool router metadata. Initial wrapper package export now writes `moeforge_config.json` and `config.json`, and `MoEForgeCarvedMLPModule` can load a carved FFN layer as a PyTorch module. `replace_hf_mlp_modules` patches tiny Llama-style causal LMs for layer and end-to-end parity checks. `eval-hf` records dense-vs-carved logits, active experts, teacher-KL, next-token NLL, latency, memory notes, warnings, package metadata, summary metrics, and per-layer dense/all-expert/selected-expert attribution across all-expert, default-pool, and document-router modes. `eval-report-html` renders self-contained HTML from eval JSON. `eval-compare` compares multiple eval JSON reports, ranks quality/speed tradeoffs, and renders active expert summaries across runs. `eval-batch` runs config-driven mode sweeps, writes per-mode JSON/HTML, emits a batch manifest, and carries recovery-eval settings forward for dense-teacher experiments. `recovery-plan` records teacher/student settings, KL loss configuration, train/eval samples, checkpoint policy, and before/after eval-batch deltas. `recovery-run` consumes that plan, promotes carved tensors to trainable parameters, computes teacher-KL/logits losses on tiny input-id batches, and writes checkpoint metadata. `recovery-export` applies checkpoint tensor state back into a recovered wrapper package. `recovery-validate` reloads recovered wrapper metadata, checks checkpoint/export compatibility, and compares original vs recovered tensor metadata. `recovery-experiment` orchestrates before eval, recovery, export, validation, after eval, and JSON/HTML comparison reports from one config, with quality trend summaries for training loss and before/after KL/NLL deltas. A tiny HF smoke recipe now exercises carve, wrapper export, eval-batch, recovery-experiment, recovery-validate, and `smoke-assert` from local JSON configs. The next step is dataset-backed smoke recipes and longer recovery schedules on a real small HF checkpoint.
