"""Test expert-level swap perturbation: swap all channels between 2 random experts."""
import sys, time
...
def _perturb_random(assignment, rng, frac=0.2):
    """Random pairwise swap perturbation."""
    non_shared = np.where(assignment != SHARED)[0]
    n_shuffle = max(2, int(frac * len(non_shared)))
    n_shuffle = n_shuffle - (n_shuffle % 2)
...
def _perturb_expert_swap(assignment, n_experts, rng):
    """Swap all channels between 2 randomly chosen experts."""
...
def _perturb_mixed(assignment, n_experts, rng, frac=0.2):
    """Alternate between expert swap and random perturbation."""
    assignment = _perturb_expert_swap(assignment, n_experts, rng)
    assignment = _perturb_random(assignment, rng, frac=0.1)
    return assignment


def converge(act, assign, n_experts, max_size, trial, n_cand):
    proxy = 0.0
    for sw in range(60):
        ...
        if n_moves == 0:
            break
    for sp in range(10):
        n_sw, assign, pn = _swap_sweep(act, assign, n_experts, 2, trial * 1000 + sp, n_cand)
        if pn > proxy:
            proxy = pn
        if n_sw == 0:
            break
...
def run(act, imp, n_experts, shared_ratio, rng, perturb_fn, time_limit=30.0):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        if best_assign is not None and trial % 2 == 1:
            ...
            assign = perturb_fn(assign, sub_rng)
        else:
            assign = _make_seed(act, imp, n_experts, shared_ratio, sub_rng)
        assign, proxy = converge(act, assign, n_experts, max_size, trial, 6)
        if proxy > best_proxy:
            best_proxy = proxy
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    perturb_fns = [
        ('random_20%', lambda a, r: _perturb_random(a, r, 0.2)),
        ('expert_swap', lambda a, r: _perturb_expert_swap(a, n_experts, r)),
        ('mixed', lambda a, r: _perturb_mixed(a, n_experts, r, 0.2)),
    ]

    for name, fn in perturb_fns:
        rng = np.random.default_rng(0)
        t0 = time.time()
        best, proxy, trials = run(act, imp, n_experts, shared_ratio, rng, fn, 30.0)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        print(f'{layer_name} perturb={name}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
