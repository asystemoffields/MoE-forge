"""Run beam search test and write results to file."""
import sys, time
...
out = open('examples/grouping-search/beam_results.txt', 'w')
...
for layer_name, data in layers:
    ...
    log(f'  standard: err={err:.6f}, proxy={best_ps:.2f}, completed={trial}, t={time.time()-t0:.1f}s')

    # Beam: probe 3 sweeps, abandon if proxy < threshold * best_so_far
    for thresh in [0.80, 0.85, 0.90, 0.95]:
        ...
        while time.time() - t0 < 22:
            ...
            # Abandon if low quality (only if we have a reference)
            if best_ps > 0 and proxy < best_ps * thresh:
                n_abandoned += 1
                trial += 1
                continue

            # Continue to convergence
            for sw in range(3, 60):
                n_moves, assign, proxy = sweep_fast(act, assign, n_experts, 2, trial*100+sw, max_size)
                if n_moves == 0: break
        ...
        err = oracle_topk_error(activations=act, down=down, assignment=best_a, top_k=2)
        counts = np.bincount(best_a[best_a != SHARED], minlength=n_experts)
        log(f'  beam(thresh={thresh}): err={err:.6f}, proxy={best_ps:.2f}, tried={trial}, abandoned={n_abandoned}, t={time.time()-t0:.1f}s')

log('\nDone.')
out.close()
print('Results written to beam_results.txt')
