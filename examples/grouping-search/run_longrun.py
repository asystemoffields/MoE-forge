"""Run with much longer time limit to see best achievable results."""
import sys, time
...
def run(act, imp, n_experts, shared_ratio, rng, time_limit=300.0):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        if best_assign is not None and trial % 2 == 1:
            ...
            assign = _perturb(assign, sub_rng, 0.2)
        else:
        ...
        if proxy > best_proxy:
            ...
            print(f'  trial {trial}: NEW BEST proxy={proxy:.2f}, elapsed={elapsed:.1f}s')
        trial += 1
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    t0 = time.time()
    best, proxy, trials = run(act, imp, 8, 0.125, rng, 120.0)
    dt = time.time() - t0
    err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
    print(f'  Final: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
