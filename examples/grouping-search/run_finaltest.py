"""Final test: compare different n_candidates and time limits."""
import sys, time
...
def run_group(act, imp, n_experts, shared_ratio, rng, n_cand, time_limit):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        assign = make_seed(act, imp, n_experts, shared_ratio, sub_rng)
        proxy = 0.0
        for sw in range(60):
            n_moves, assign, proxy = sweep_standard(act, assign, n_experts, 2, trial*100+sw, max_size)
            if n_moves == 0: break
        for sp in range(10):
            n_sw, assign, pn = sweep_swap(act, assign, n_experts, 2, trial*1000+sp, n_cand)
            if pn > proxy: proxy = pn
            if n_sw == 0: break
            for sw in range(30):
                n_moves, assign, p2 = sweep_standard(act, assign, n_experts, 2, trial*10000+sp*30+sw, max_size)
...
out = open('examples/grouping-search/finaltest_results.txt', 'w')
def log(s):
    out.write(s + '\n')
    out.flush()
...
for layer_name, data in layers:
    ...
    log(f'\n=== {layer_name} ===')
    for n_cand in [4, 6, 8, 10, 12]:
        for tl in [22, 30, 45]:
            rng = np.random.default_rng(0)
            t0 = time.time()
            best_assign, best_proxy, n_trials = run_group(act, imp, 8, 0.125, rng, n_cand, tl)
            dt = time.time() - t0
            err = oracle_topk_error(activations=act, down=down, assignment=best_assign, top_k=2)
            log(f'  n_cand={n_cand}, tl={tl}s: err={err:.6f}, proxy={best_proxy:.2f}, trials={n_trials}, dt={dt:.1f}s')
