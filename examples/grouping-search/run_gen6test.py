"""Test gen6 candidate: n_cand=6 + perturbation restart, verify across multiple seeds."""
import sys, time
...
def run_gen6(act, imp, n_experts, shared_ratio, rng, time_limit=30.0):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        # Alternate: fresh seed every other trial, perturb from best otherwise
        if best_assign is not None and trial % 2 == 1:
            assign = best_assign.copy()
            assign = _perturb(assign, sub_rng, frac=0.2)
        else:
            assign = _make_seed(act, imp, n_experts, shared_ratio, sub_rng)

        assign, proxy = converge(act, assign, n_experts, max_size, trial, n_cand=6)
...
# Test across 3 different starting seeds to check robustness
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for seed in [0, 1, 42]:
        ...
        t0 = time.time()
        best, proxy, trials = run_gen6(act, imp, 8, 0.125, rng, 30.0)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        errs.append(err)
        print(f'{layer_name} seed={seed}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
    print(f'{layer_name} mean={sum(errs)/len(errs):.6f}')
