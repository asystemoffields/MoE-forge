"""Test different top_n values for targeted exhaustive swap, and combined strategy."""
import sys, time
...
def run_with_targeted(act, imp, n_experts, shared_ratio, rng, n_cand, targeted_n, time_limit=30.0):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        # Targeted exhaustive swap
        for sp in range(5):
            n_sw, assign, pn = _targeted_swap_sweep(act, assign, n_experts, 2, trial * 99999 + sp, top_n=targeted_n)
            if pn > proxy:
                proxy = pn
            if n_sw == 0:
                break
            for sw in range(20):
                n_moves, assign, p2 = _sweep(act, assign, n_experts, 2, trial * 1000000 + sp * 20 + sw, max_size)
                if p2 > proxy:
                    proxy = p2
                if n_moves == 0:
                    break
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for targeted_n in [30, 50, 80]:
        rng = np.random.default_rng(0)
        t0 = time.time()
        best, proxy, trials = run_with_targeted(act, imp, 8, 0.125, rng, 6, targeted_n, 30.0)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        print(f'{layer_name} targeted_n={targeted_n}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
