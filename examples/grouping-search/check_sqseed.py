"""Verify sq_cosine seed produces valid assignments and check what changed."""
import sys, time
...
def _make_seed_sq_cosine(activations, importance, n_experts, shared_ratio, rng):
    ...
    vecs = (activations[:, remaining] ** 2).T
...
assign = _make_seed_sq_cosine(act, imp, n_experts, shared_ratio, sub_rng)

# Check balance after seed
unique, counts = np.unique(assign[assign != SHARED], return_counts=True)
print(f'After seed: experts used={len(unique)}, counts={counts}, max={counts.max()}, max_size={max_size}')
print(f'Balance ok: {counts.max() <= max_size}')
...
unique, counts = np.unique(assign[assign != SHARED], return_counts=True)
print(f'Final: proxy={proxy:.2f}, experts={len(unique)}, max_count={counts.max()}')
print(f'All constraints ok: {len(unique) == n_experts and counts.max() <= max_size}')
