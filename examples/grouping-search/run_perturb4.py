"""Compare perturbation variants with n_cand=6 (faster per restart) at 30s budget."""
import sys, time
...
def converge6(act, assign, n_experts, max_size, trial):
    proxy = 0.0
    for sw in range(60):
...
def run_test(act, imp, n_experts, shared_ratio, rng, time_limit=30.0, perturb_frac=0.2, perturb_every=2):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        assign, proxy = converge6(act, assign, n_experts, max_size, trial)
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for frac, every in [(0.1, 2), (0.15, 2), (0.2, 2), (0.25, 2), (0.3, 2), (0.2, 3)]:
        rng = np.random.default_rng(0)
        t0 = time.time()
        best, proxy, trials = run_test(act, imp, 8, 0.125, rng, 30.0, frac, every)
        dt = time.time() - t0
