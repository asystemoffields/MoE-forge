"""Test the hybrid refinement on layer9."""
import sys, time
...
def run_hybrid_sweep(act, down, assign, contribs, norms_sq, shared_contrib,
    ...
    for i in channel_order:
        ...
        # Verify with exact oracle error
        dst = best_dst
        p_dst = p_all[dst]
...
def refine(act, down, assign, n_experts=8, top_k=2, n_sweeps=15, time_limit=25):
    ...
    for sweep in range(n_sweeps):
        ...
        dt = time.time() - t0
        print(f'  Sweep {sweep+1}: {n_moves} moves, err={curr_err:.6f}, {dt:.2f}s')
        if n_moves == 0:
            break
        if time.time() - t_total > time_limit:
            break
...
for layer_name in ['layer3', 'layer9']:
    ...
    t0 = time.time()
    assign_ref = refine(act, down, assign, time_limit=25)
    print(f'  Total: {time.time()-t0:.1f}s')

    err = oracle_topk_error(activations=act, down=down, assignment=assign_ref, top_k=2)
    print(f'  Final oracle error: {err:.6f}')
