"""Check variance across restarts: what proxy values are being found?"""
import sys, time
...
def converge(act, assign, n_experts, max_size, trial):
    proxy = 0.0
    for sw in range(60):
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    print(f'\n{layer_name}: Independent restart proxies (seed=0, trials 0-9)')
    ...
    for trial in range(10):
        ...
        assign, proxy = converge(act, assign, n_experts, max_size, trial)
        err = oracle_topk_error(activations=act, down=down, assignment=assign, top_k=2)
        proxies.append(proxy)
        errs.append(err)
        print(f'  trial {trial}: proxy={proxy:.2f}, err={err:.6f}')
    print(f'  Best proxy: {max(proxies):.2f}, best err: {min(errs):.6f}')
    print(f'  Mean proxy: {sum(proxies)/len(proxies):.2f}, mean err: {sum(errs)/len(errs):.6f}')
