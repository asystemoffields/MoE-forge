...
def sweep_swap(activations, assignment, n_experts, top_k, rng_seed, n_candidates=2):
    T, I = activations.shape
    act_sq = activations ** 2
    exp_sq = np.zeros((n_experts, T))
    for e in range(n_experts):
...
out = open('examples/grouping-search/paramtune_results.txt', 'w')
def log(s):
    out.write(s + '\n')
    out.flush()
...
for layer_name, data in layers:
    ...
    # Test n_candidates=1 vs 2 vs 4 (within 22s each)
    for n_cand in [1, 2, 4]:
        ...
        while time.time() - t0 < 22:
            ...
            assign = make_seed(act, imp, 8, 0.125, sub)
            proxy = 0.0
            for sw in range(60):
                n_moves, assign, proxy = sweep_standard(act, assign, 8, 2, trial*100+sw, max_size)
                if n_moves == 0: break
            for sp in range(10):
                n_sw, assign, pn = sweep_swap(act, assign, 8, 2, trial*1000+sp, n_cand)
                if pn > proxy: proxy = pn
                if n_sw == 0: break
                for sw in range(30):
                    n_moves, assign, p2 = sweep_standard(act, assign, 8, 2, trial*10000+sp*30+sw, max_size)
        ...
        counts = np.bincount(best_assign[best_assign != SHARED], minlength=8)
        log(f'  n_cand={n_cand}: err={err:.6f}, proxy={best_ps:.2f}, restarts={trial}, t={time.time()-t0:.1f}s')
