"""Test multi-restart k-means seeding to get better initial partitions."""
import sys, time
...
def _kmeans(points, k, rng, iters=25):
    ...
    for _ in range(iters):
        ...
        new_inertia = distances.min(axis=1).sum()
        for c in range(k):
...
def _make_seed_best_kmeans(activations, importance, n_experts, shared_ratio, rng, n_restarts=3):
    """Run k-means n_restarts times, pick assignment with lowest inertia."""
    ...
    for _ in range(n_restarts):
        labels, centers, inertia = _kmeans(vecs, n_experts, rng)
        if inertia < best_inertia:
            best_inertia = inertia
            best_labels = labels

    for ch, lab in zip(remaining, best_labels):
        assignment[ch] = int(lab)
    return assignment


def _make_seed_cosine(activations, importance, n_experts, shared_ratio, rng):
    I = importance.shape[0]
    n_shared = int(round(shared_ratio * I))
    order = np.argsort(-importance)
    assignment = np.empty(I, dtype=int)
    ...
    vecs = activations[:, remaining].T
    vecs = vecs / (np.linalg.norm(vecs, axis=1, keepdims=True) + 1e-12)
    labels, _, _ = _kmeans(vecs, n_experts, rng)
...
def run_multikmeans(act, imp, n_experts, shared_ratio, rng, n_km, perturb_frac, time_limit=30.0):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        if best_assign is not None and trial % 2 == 1:
            ...
            assign = _perturb(assign, sub_rng, frac=perturb_frac)
        else:
            assign = _make_seed_best_kmeans(act, imp, n_experts, shared_ratio, sub_rng, n_km)
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for n_km, frac in [(1, 0.2), (3, 0.2), (5, 0.2)]:
        rng = np.random.default_rng(0)
        t0 = time.time()
        best, proxy, trials = run_multikmeans(act, imp, 8, 0.125, rng, n_km, frac, 30.0)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        print(f'{layer_name} n_km={n_km} frac={frac}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
