"""Test perturbation from best solution: randomly swap chunks of channels between experts."""
import sys, time
...
def _perturb(assignment, n_experts, rng, frac=0.1):
    """Randomly shuffle frac of routed channels (balance-preserving pairwise swaps)."""
    non_shared = np.where(assignment != SHARED)[0]
    n_shuffle = max(2, int(frac * len(non_shared)))
    n_shuffle = n_shuffle - (n_shuffle % 2)  # Make even for pairs
...
def run_perturb(act, imp, n_experts, shared_ratio, rng, n_cand, time_limit=30.0, perturb_frac=0.1):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
    ...
    while time.time() - t_start < time_limit:
        ...
        if best_assign is not None and trial > 0 and trial % 2 == 1:
        ...
        assign, proxy = converge(act, assign, n_experts, max_size, trial, n_cand)
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for frac in [0.05, 0.1, 0.2]:
        ...
        t0 = time.time()
        best, proxy, trials = run_perturb(act, imp, 8, 0.125, rng, 6, 30.0, frac)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        print(f'{layer_name} frac={frac}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
