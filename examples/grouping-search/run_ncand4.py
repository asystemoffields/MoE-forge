"""Test n_cand=4 with more restarts and perturbation."""
import sys, time
...
def run(act, imp, n_experts, shared_ratio, rng, n_cand, time_limit=30.0, perturb_frac=0.2):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
...
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for n_cand, frac in [(4, 0.2), (4, 0.3), (6, 0.2), (4, 0.0)]:
        rng = np.random.default_rng(0)
        t0 = time.time()
        best, proxy, trials = run(act, imp, 8, 0.125, rng, n_cand, 30.0, frac)
        dt = time.time() - t0
        err = oracle_topk_error(activations=act, down=down, assignment=best, top_k=2)
        print(f'{layer_name} n_cand={n_cand} frac={frac}: err={err:.6f}, proxy={proxy:.2f}, trials={trials}, dt={dt:.1f}s')
