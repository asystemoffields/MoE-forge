"""Gen6b: Test perturb + adaptive sweep (n_cand=[6,10]) at different time limits."""
import sys, time
...
def converge_adaptive(act, assign, n_experts, max_size, trial):
    ...
    for outer_phase in range(10):
        ...
        for cand_idx, n_cand in enumerate([6, 10]):
            n_swaps, assign, pn = _swap_sweep(
                act, assign, n_experts, 2,
                trial * 1000 + outer_phase * 100 + cand_idx, n_cand)
            if pn > proxy:
                proxy = pn
            if n_swaps > 0:
                improved = True
                for sw in range(30):
                    n_moves, assign, p2 = _sweep(
                        act, assign, n_experts, 2,
                        trial * 100000 + outer_phase * 1000 + cand_idx * 30 + sw,
                        max_size)
                    if p2 > proxy:
                        proxy = p2
                    if n_moves == 0:
...
def run(act, imp, n_experts, shared_ratio, rng, time_limit=30.0, perturb_frac=0.2):
    T, I = act.shape
    n_shared = int(round(shared_ratio * I))
    n_routed = I - n_shared
...
# Test with seed=0 (what eval uses)
for layer_name, data in [('layer3', d3), ('layer9', d9)]:
    ...
    for frac in [0.1, 0.2, 0.3]:
        ...
        t0 = time.time()
        best, proxy, trials = run(act, imp, 8, 0.125, rng, 30.0, frac)
        dt = time.time() - t0
